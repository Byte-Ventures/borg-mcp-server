...
The current public preview is `borgmcp-server@0.1.11`, published on npm.
...
published `0.1.11` package adds non-expiring enrolled sessions, authoritative
runtime status, safe legacy-runtime updates, and bounded managed-service stop
behavior, and consumes the audited exact `borgmcp-shared@0.4.3` registry release.
Current source is the unpublished `borgmcp-server@0.1.12` release candidate,
which prepares the verified runtime with npm 12 while retaining npm 11 behavior.
Version `0.1.11` remains the install target until the candidate passes exact-SHA
...
Version `0.1.11` completed a fresh exact-source, tagged-artifact, tokenless OIDC
