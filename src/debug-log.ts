...
export type DebugEvent =
  ...
  | { readonly event: "request"; readonly route: DebugRoute; readonly method: string; readonly authentication: "not_required" | "missing" | "invalid" | "revoked" | "evicted" | "rejected" | "cube-deleted" | "accepted"; readonly authorization: "not_checked" | "accepted" | "denied_or_not_found"; readonly principal?: Principal; readonly status: number; readonly durationMs: number }
...
const DEBUG_ROUTES: readonly DebugRoute[] = ["health", "protocol", "runtime", "enrollment_exchange", "client_attach", "repository_cube_resolve", "repository_cube_association", "cubes", "cube", "cube_roles", "cube_role", "cube_role_section_patch", "cube_taxonomy_patch", "cube_drones", "cube_drone_self_metadata", "cube_logs", "cube_log_entry", "cube_ack_status", "cube_acks", "cube_decisions", "cube_documents", "cube_stream", "unknown"];
const HTTP_METHODS = ["GET", "POST", "PUT", "PATCH", "DELETE", "OTHER"] as const;
const AUTH_RESULTS = [
  "not_required", "missing", "invalid", "revoked", "evicted", "rejected", "cube-deleted", "accepted",
] as const;
